                                ...
                                <tbody>
                                    ...
                                    <c:forEach var="item" items="${cartItems}">
                                        <tr>
                                            ...
                                            <td><fmt:formatNumber value="${item.price- item.price*discount}" maxFractionDigits="1" /></td>
                                            <td>${item.quantity}</td>
                                            <td><fmt:formatNumber value="${(item.quantity * item.product.price) - (item.quantity * item.product.price*discount)}" maxFractionDigits="2" /></td>
                                        </tr>
                                    </c:forEach>
                                </tbody>
                            </table>
                        </div>
                        <input type="text"  hidden=""name="discount" value="${discount}">
                        <c:set var="subtotal" value="0" />
                        <c:forEach var="item" items="${sessionScope.cart.items}">
                            <c:set var="subtotal" value="${subtotal + (item.quantity * item.product.price) - (item.quantity * item.product.price*discount)}" />
                        </c:forEach>
